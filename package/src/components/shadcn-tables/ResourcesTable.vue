...
function resourceImageUrl(resource: Resource): string | null {
  if (!resource?.image) return null
  return `${API_URL}/uploads/${resource.image}`
}
...
function fetchResources() {
  ...
  error.value = null
  axios
    .get(`${API_URL}/api/resources`, { headers: authHeaders() })
    .then((res) => { resources.value = Array.isArray(res.data) ? res.data : [] })
    .catch((err) => {
      resources.value = []
...
async function submitEdit(e: Event) {
  ...
  try {
    ...
    if (idx !== -1) resources.value[idx] = res.data
    closeEdit()
  } catch (err: any) {
...
async function onImageChange(e: Event) {
  ...
  try {
    ...
    const res = await axios.post(
      `${API_URL}/api/resources/${editingResource.value.id}/image`,
      formData,
      { headers: { ...authHeaders(), 'Content-Type': 'multipart/form-data' } }
    )
    if (editingResource.value) {
      editingResource.value = { ...editingResource.value, image: res.data.image }
    }
    const idx = resources.value.findIndex((r) => r.id === editingResource.value?.id)
    if (idx !== -1) resources.value[idx] = { ...resources.value[idx], image: res.data.image }
  } catch (err: any) {
...
async function submitDelete() {
  ...
  try {
    await axios.delete(`${API_URL}/api/resources/${resource.id}`, { headers: authHeaders() })
    closeDelete()
    fetchResources()
  } catch (err: any) {
...
<template>
  <div class="space-y-4">
    <p class="text-muted-foreground text-sm">
      Gérez les ressources de base : nom, description et image.
    </p>
    ...
    <Card class="border shadow-none p-0">
      <Table>
        ...
        <TableBody>
          ...
          <TableRow v-else-if="resources.length === 0">
            <TableCell colspan="4" class="text-center py-8 text-muted-foreground">
              Aucune ressource. Cliquez sur « Créer une ressource » pour en ajouter.
            </TableCell>
          </TableRow>
          <TableRow
            v-else
          ...
          >
            <TableCell class="w-20">
              <div class="h-12 w-12 rounded-md overflow-hidden bg-muted flex items-center justify-center">
                <img
                  v-if="resourceImageUrl(resource)"
            ...
            <TableCell class="text-end">
              <div class="flex justify-end gap-2">
                <Button variant="outline" size="sm" @click="openEdit(resource)" title="Modifier">
                  <Pencil class="h-4 w-4" />
                </Button>
                <Button variant="outline" size="sm" class="text-destructive" @click="confirmDelete(resource)" title="Supprimer">
    ...
    <Dialog v-model:open="createOpen">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Créer une ressource de base</DialogTitle>
        </DialogHeader>
        <form @submit="submitCreate" class="space-y-4">
          <div class="space-y-2">
            <Label for="create-name">Nom</Label>
            <Input id="create-name" v-model="createName" placeholder="Ex. Minerai, Bois..." required />
          </div>
          <div class="space-y-2">
    ...
    <Dialog v-model:open="editOpen" @update:open="(v: boolean) => !v && closeEdit()">
      <DialogContent class="sm:max-w-md">
        ...
        <form v-if="editingResource" @submit="submitEdit" class="space-y-4">
          <input
            ref="imageUploadRef"
            type="file"
            accept="image/png,image/jpeg,image/jpg,image/gif,image/webp"
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-4">
              <div class="h-16 w-16 rounded-md overflow-hidden bg-muted flex items-center justify-center shrink-0">
                <img
                  v-if="resourceImageUrl(editingResource)"
                  :src="resourceImageUrl(editingResource)!"
