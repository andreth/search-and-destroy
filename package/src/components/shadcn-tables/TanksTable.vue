...
const deleteTarget = ref<Tank | null>(null)
const deleteDialogOpen = ref(false)
const deleteLoading = ref(false)
const deleteError = ref<string | null>(null)

function tankImageUrl(tank: Tank): string | null {
  if (!tank?.image) return null
  return `${API_URL}/uploads/${tank.image}`
}
...
function fetchTanks() {
  ...
  error.value = null
  axios
    .get(`${API_URL}/api/tanks`, { headers: authHeaders() })
    .then((res) => { tanks.value = Array.isArray(res.data) ? res.data : [] })
    .catch((err) => {
      tanks.value = []
...
async function submitCreate(e: Event) {
  ...
  try {
    ...
    await axios.post(`${API_URL}/api/tanks`, formData, {
...
function openEdit(tank: Tank) {
  ...
  editName.value = tank.name
  editDescription.value = tank.description ?? ''
...
async function onImageChange(e: Event) {
  const target = e.target as HTMLInputElement
  const file = target.files?.[0]
  if (!file || !editingTank.value) return
  imageUploadError.value = null
  ...
  try {
    ...
    const res = await axios.post(
      `${API_URL}/api/tanks/${editingTank.value.id}/image`,
      formData,
      { headers: { ...authHeaders(), 'Content-Type': 'multipart/form-data' } }
    )
    if (editingTank.value) {
      editingTank.value = { ...editingTank.value, image: res.data.image }
    }
    const idx = tanks.value.findIndex((t) => t.id === editingTank.value?.id)
    if (idx !== -1) tanks.value[idx] = { ...tanks.value[idx], image: res.data.image }
  } catch (err: any) {
...
function confirmDelete(tank: Tank) {
  deleteTarget.value = tank
...
async function submitDelete() {
  const tank = deleteTarget.value
  if (!tank) return
  deleteError.value = null
  deleteLoading.value = true
  try {
    await axios.delete(`${API_URL}/api/tanks/${tank.id}`, { headers: authHeaders() })
    closeDelete()
    fetchTanks()
  } catch (err: any) {
...
onMounted(() => fetchTanks())
</script>

<template>
  <div class="space-y-4">
    <p class="text-muted-foreground text-sm">
      Gérez les unités de type tank : nom, description et image.
    </p>
    ...
    <Card class="border shadow-none p-0">
      <Table>
        ...
        <TableBody>
          ...
          <TableRow v-else-if="tanks.length === 0">
            <TableCell colspan="4" class="text-center py-8 text-muted-foreground">
          ...
          >
            <TableCell class="w-20">
              <div class="h-12 w-12 rounded-md overflow-hidden bg-muted flex items-center justify-center">
                <img
                  v-if="tankImageUrl(tank)"
                  :src="tankImageUrl(tank)!"
                  :alt="tank.name"
            ...
            <TableCell class="font-medium">{{ tank.name }}</TableCell>
            <TableCell class="text-muted-foreground text-sm max-w-xs truncate">
              {{ tank.description || '–' }}
            </TableCell>
            <TableCell class="text-end">
              <div class="flex justify-end gap-2">
                <Button variant="outline" size="sm" @click="openEdit(tank)" title="Modifier">
                  <Pencil class="h-4 w-4" />
                </Button>
                <Button variant="outline" size="sm" class="text-destructive" @click="confirmDelete(tank)" title="Supprimer">
    ...
    <Dialog v-model:open="createOpen">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Créer une unité tank</DialogTitle>
        </DialogHeader>
        <form @submit="submitCreate" class="space-y-4">
          <div class="space-y-2">
            <Label for="create-name">Nom</Label>
            <Input id="create-name" v-model="createName" placeholder="Ex. Char Léger" required />
          </div>
          <div class="space-y-2">
    ...
    <Dialog v-model:open="editOpen" @update:open="(v: boolean) => !v && closeEdit()">
      <DialogContent class="sm:max-w-md">
        ...
        <form v-if="editingTank" @submit="submitEdit" class="space-y-4">
          <input
            ref="imageUploadRef"
            type="file"
            accept="image/png,image/jpeg,image/jpg,image/gif,image/webp"
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-4">
              <div class="h-16 w-16 rounded-md overflow-hidden bg-muted flex items-center justify-center shrink-0">
                <img
                  v-if="tankImageUrl(editingTank)"
                  :src="tankImageUrl(editingTank)!"
                  :alt="editingTank.name"
                  class="h-full w-full object-cover"
                />
                <ImageIcon v-else class="h-8 w-8 text-muted-foreground" />
              </div>
